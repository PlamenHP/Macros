...
namespace Macros.Domain.Common
{
    public static class Guard
    {
        ...
        internal static void ForStringLength<T>(string name, object minNameLength, object maxNameLength, string v)
        {
        ...
        public static void AgainstOutOfRange<TException>(double number, int min, int max, string name = "Value")
            where TException : BaseDomainException, new()
        {
            if (min <= number && number <= max)
            {
